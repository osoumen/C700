...
void DspController::onDeviceAdded(void *ref)
{
    ...
    // N8かチェック
    /*
    if (This->mEdDev.CheckConnectN8() == false) {
        return;
    }*/
    ...
    This->mEdDev.Reset();
    /*
    This->mEdDev.LoadFirmN8(s_firmData, sizeof(s_firmData));
    int mapper = This->mEdDev.LoadRomN8(s_romData, sizeof(s_romData), true);
    
    This->mEdDev.StartGame(mapper);
    */
...
{
    addr &= 0x7f;
    bool doWrite = (mDspMirror[addr] != data)?true:false;
...
void DspController::doWriteDspHw(int addr, unsigned char data)
{
    if (mIsHwAvailable) {
         ...
         */
        mEdDev.BlockWrite(addr & 0xff, data);
        //mEdDev.WriteAndWait(0, mPort0stateHw);
        mPort0stateHw = mPort0stateHw ^ 0x01;
        //mSpcDev.WriteBuffer();
